...
//        private void IblTodayAttendance_Click(object sender, EventArgs e)
//        {
...
namespace Gym_Management_System
{
    public partial class Reports : Form
    {
        ...
        private void LoadSummaryCards()
        {
            IblTodayAttendance.Text = _reportService.GetTodayAttendanceCount().ToString();
            lblActiveSubscriptions.Text = _reportService.GetActiveSubscriptionsCount().ToString();
            lblMonthlyRevenue.Text = _reportService.GetMonthlyRevenue().ToString("N0") + " EGP";
        }
        ...
        private void btnAttendance_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataGridView1.Columns["TraineeName"] != null)
                    dataGridView1.Columns["TraineeName"].HeaderText = "Trainee Name";
                if (dataGridView1.Columns["Date"] != null)
                    dataGridView1.Columns["Date"].HeaderText = "Entry Date";

                IblTodayAttendance.Text = "Count: " + data.Count;
                lblTodayAttendance.Text = "Attendance Report";
            }
            catch (Exception ex)
            {
        ...
        private void btnActiveSubscriptions_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                dataGridView1.Rows.Add("Active Subscriptions Count", count);
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void btnExpiredSubscriptions_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataGridView1.Columns["SubscriptionName"] != null)
                    dataGridView1.Columns["SubscriptionName"].HeaderText = "Subscription";
                if (dataGridView1.Columns["EndDate"] != null)
                ...
                IblTodayAttendance.Text = "Count: " + data.Count;
                lblTodayAttendance.Text = "Expired Subscriptions";
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void btnTrainers_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (dataGridView1.Columns["TraineeCount"] != null)
                    dataGridView1.Columns["TraineeCount"].HeaderText = "Trainee Count";
                if (dataGridView1.Columns["HireDate"] != null)
                    dataGridView1.Columns["HireDate"].HeaderText = "Hire Date";

                IblTodayAttendance.Text = "Count: " + data.Count;
                lblTodayAttendance.Text = "Trainers Report";
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
        ...
        private void btnRevenue_Click(object sender, EventArgs e)
        {
            ...
            {
                decimal revenue = _reportService.GetMonthlyRevenue();
                lblMonthlyRevenue.Text = revenue.ToString("N0") + " EGP";

                dataGridView1.DataSource = null;
                dataGridView1.Columns.Clear();
                dataGridView1.Columns.Add("Month", "Month");
                dataGridView1.Columns.Add("Revenue", "Total Revenue");
                dataGridView1.Rows.Add(
                    DateTime.Today.ToString("MMMM yyyy"),
                    revenue.ToString("N0") + " EGP"
                );

                lblTodayAttendance.Text = "Current Month Revenue";
                IblTodayAttendance.Text = DateTime.Today.ToString("MMMM yyyy");
            }
            catch (Exception ex)
            {
                MessageBox.Show("Error: " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
